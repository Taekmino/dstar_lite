...
bool OccupancyGridMap::isEmpty(const std::pair<int, int> p) {
    return occupancy_map[p.first][p.second] != OBSTACLE;
    }
...
Vertices ScanMap::updateChangedCosts(const std::unordered_map<std::pair<int, int>, int, pair_hash>& local_grid) {
    ...
    //     if (value > 60) {                         //value가 100인데(obstacle인데)
    //         if (slam_map.isEmpty(node)) {        //slam_map node의 value가 100이 아니면
    //             Vertex v(node);
    //             auto succ = slam_map.getSuccessors(node);
    //             for (auto& u : succ) {
    //                 v.addEdgeWithCost(u, c(u, node));
    //             }
    //             vertices.addVertex(v);
    //             slam_map.setObstacle(node);
    //         }
    //     }
    //     else {                                  //value가 100이 아닌데(obstacle이 아닌데)
    //         if (!slam_map.isEmpty(node)) {     //scan map node의 value가 100이면 
    //             Vertex v(node);
    //             auto succ = slam_map.getSuccessors(node);
    //             for (auto& u : succ) {
    //                 v.addEdgeWithCost(u, c(u, node));
